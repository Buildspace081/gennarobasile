...
Keep information density deliberately low. Each viewport should have one primary message or action. The homepage sequence is Intro, Experience, Software, Hardware and Buildspace081 (last, before the footer). There is no separate homepage "About" section — the Intro carries the short bio plus a "Read the full story" link to `/about`, and the header's "About" nav item links directly to `/about`. Experience, however, *is* a full homepage section again (with the complete `ExperienceTabs` breakdown, not a teaser): the user first moved it to `/about`-only, then explicitly reversed that decision, reasoning that an overview of his path needs to come before the Projects/Software section so a visitor has context first. `ExperienceTabs` lives only on the homepage now, not on `/about` — do not duplicate it there again without being asked. The mechanical-engineering degree is shown with only its graduation year ("2020"), not an enrolment-to-graduation range — an intentional, honest framing choice by the user, not an error to "fix" back to a date range. Every `Experience` entry (and sub-`Role`) should carry a short, honest "What I learned" line in addition to its action bullets — per the user's explicit instruction to keep the content descriptive and to highlight what he learned at each stage, not just what he did; this reframes past (non-Data-Science) experience as transferable groundwork rather than a detour. The first viewport uses a two-column personal introduction on desktop: a reserved portrait area on the left and the opening bio on the right. The header keeps the name and primary navigation fully decomposed inline on the left (never collapsed into a dropdown on desktop) and social/contact icons (email, GitHub, LinkedIn, Substack) at the far right; on mobile the portrait stacks above the introduction and the nav collapses into a disclosure menu only below the `lg` breakpoint. Study resources must not appear on the homepage or primary navigation; existing study routes remain a non-primary technical archive. Do not display controls, filters or metadata unless they are useful and functional.

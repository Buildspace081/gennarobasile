...
The visual concept is a clean, bright personal engineering portfolio: readability and recruiter scannability come first. The background is one continuous, uniform white (`--color-paper`); do not add grid patterns, textures or alternating section background blocks. Separate sections with generous vertical space, explicit headings, subtle gradient rules and restrained scroll-entry fades. Use consistent light neutral surfaces only for contained content such as project cards. Use the Ink, Paper, Purple, Blue, Orange, Line and Muted tokens defined in `global.css` — Ink is near-black text, Paper is white, Blue is a deep teal accent, Orange is a warm amber/rust accent, Purple is a muted terracotta/clay accent (no violet or lavender tones). Body and UI text use Inter, a friendly, highly legible sans-serif; do not reintroduce a serif or mixed-font system.

Keep information density deliberately low. Each viewport should have one primary message or action. The homepage sequence is Intro, Experience, Software, Hardware and Buildspace081 (last, before the footer); there is no separate homepage "About" section — the Intro carries the short bio plus a "Read the full story" link to `/about`, and the header's "About" nav item links directly to `/about`. The Experience section's content (heading and tabs) is centred in a narrower column within the shell, not stretched edge to edge, so it reads as its own centred block with visible margins on both sides. The first viewport uses a two-column personal introduction on desktop: a reserved portrait area on the left and the opening About narrative on the right. The header keeps the name and primary navigation fully decomposed inline on the left (never collapsed into a dropdown on desktop) and social/contact icons (email, GitHub, LinkedIn, Substack) at the far right; on mobile the portrait stacks above the introduction and the nav collapses into a disclosure menu only below the `lg` breakpoint. Study resources must not appear on the homepage or primary navigation; existing study routes remain a non-primary technical archive. Experience uses accessible selectable panels, ordered most-recent-first (education and roles interleaved), with the current Data Science master's as the default-selected, topmost chapter and the mechanical-engineering degree last. Do not display controls, filters or metadata unless they are useful and functional.

Do not add generic purple-blue gradients, glassmorphism, glowing globes, stock robots, fake terminals, decorative circuit boards, excessive neural-network graphics, SaaS card walls, violet/lavender tones or motion that delays navigation. The personal name is primary. Reuse `BaseLayout`, `Header`, `ExperienceTabs`, `ProjectCard` and `ResourceCard`.

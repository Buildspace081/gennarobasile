...
- The user is changing careers (mechanical engineering/automotive → Data Science/AI) and felt the full Experience breakdown didn't belong front-and-centre on the homepage. Rather than delete it, `ExperienceTabs` was moved into `/about` under a new `#experience` section ("The path so far.", same centred/narrow styling as before), and the homepage's `#experience` section was replaced with a short teaser paragraph (naming Ecoindustria, Maserati, Stellantis briefly, framed as transferable habits) plus a "See the full path →" link to `/about#experience`. `index.astro` no longer imports `ExperienceTabs`; `about.astro` does. The header's "Experience" nav item still points to the homepage's `/#experience` anchor (the teaser), not directly to the About page section.
...
- The Experience section's heading and tabs are wrapped in a `mx-auto max-w-2xl` column (narrowed further from an earlier `max-w-4xl` pass) instead of spanning the full 1180px shell, so the block reads as centred on the page with more visible whitespace on both sides; the heading text is centred, the tab list/panel below it are not. Text sizes inside were reduced to match the narrower column: the section heading is now `text-3xl`/`sm:text-4xl` (was the large shared `.section-title`), and inside `ExperienceTabs` the role/degree title is `text-2xl` (was `text-3xl`), organisation `text-base` (was `text-lg`), tab labels `text-sm` (was `text-base`), and bullet/point text `.95rem` (was `body-copy`'s larger clamp size).

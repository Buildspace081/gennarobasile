...
Last updated: 2026-09-17 — Visual identity switched from the dark cutting-mat theme to a bright, uniform-white theme with the Inter typeface, then further simplified to monochrome (black text/bold emphasis, no accent colours) at the user's request; the header now shows fully decomposed navigation plus email/GitHub/LinkedIn/Substack icons; the homepage's separate "About" section was removed entirely (folded into Intro, which now links to `/about`); the whole site's type scale and layout width were reduced and every homepage/about section is now centred in a narrow column with wider side margins; the mechanical-engineering degree now shows only its graduation year (2020); Byte.ml was removed from the site at the user's request. **Experience churn, resolved this session:** it was moved off the homepage entirely (full breakdown to `/about#experience`, then even the teaser removed), but the user then reconsidered and asked for it back on the homepage, in its original position before Software — reasoning that a visitor needs an overview of his path before reaching the Projects section. `ExperienceTabs` is homepage-only again (not on `/about`), and every entry now includes an explicit "What I learned" line per the user's request to keep the content descriptive and highlight learning outcomes, not just duties — treat this as the current, settled state.
...
- Homepage sections now follow Intro, Experience, Software, Hardware and Buildspace081 (no separate "About" section — see below).
